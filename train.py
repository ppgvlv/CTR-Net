...
def main():
    ...
    # args = option.get_args_parser().parse_args()
    ...
    model = CTR_Net.create_model(nb_cls=args.nb_cls, img_size=args.img_size[::-1], use_2d_rope=True, rope_mode='axial', rope_theta=100.0)
...
def compute_loss(args, model, image, batch_size, criterion, text, length):
    # 如果你想 mask 逻辑生效
    foreground_mask = torch.ones(batch_size, model.num_patches, device=image.device)  # 全 1 张量
    preds = model(image, mask_ratio=args.mask_ratio, foreground_mask=foreground_mask, use_masking=True)
