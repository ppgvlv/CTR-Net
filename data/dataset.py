...
def get_images(fname, max_w=500, max_h=500, nch=1):
    try:
        ...
        pad_w = max(0, max_w - image_data.shape[1])
        image_data = np.pad(image_data, ((0, 0), (0, pad_w), (0, 0)),
                            mode='constant', constant_values=1.0)

    except Exception as e:
        print('[Warning] Could not read:', fname, ':', e)
        image_data = np.ones((max_h, max_w, nch), dtype=np.float32)  # 返回空白图
